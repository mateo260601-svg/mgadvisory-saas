...
from app.services.output_service import build_im_deck, build_lender_deck
...
@router.post("/projects/{project_id}/decks/im/plan")
def api_plan_im_deck(project_id: str):
    if not get_project(project_id):
        raise HTTPException(status_code=404, detail="Project not found")
    return build_deck_blueprint(project_id, deck_type="im", force=True)
...
@router.get("/projects/{project_id}/decks/im/blueprint")
def api_get_im_blueprint(project_id: str):
    if not get_project(project_id):
        raise HTTPException(status_code=404, detail="Project not found")
    return load_deck_blueprint(project_id, deck_type="im")
...
@router.post("/projects/{project_id}/decks/im/generate")
def api_generate_im_deck(project_id: str):
    project = get_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    output = build_im_deck(project_id)
    return {"output": output}
...
@router.get("/projects/{project_id}/decks/im/download")
def api_download_im_deck(project_id: str):
    project = get_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    output = build_im_deck(project_id)
